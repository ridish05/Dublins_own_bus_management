...
public class TernaryTreeClass
{
     ...
     * @return: Associated with the key if found, -1 if not found.
     */
    protected int get(TreeLeaf node, String key)
    {
        char c = key.charAt(0);
        if (node == null)
        {   
     ...
     * @return: void.
     */
    private void traverse(TreeLeaf node, String string)
    {
        if (node != null)
        {
     ...
     * @return: A TSTNode.
     */
    protected TreeLeaf put(TreeLeaf node, String key, int value)
    {
        char c = key.charAt(0);
        if (key.length() > 1)
        {
