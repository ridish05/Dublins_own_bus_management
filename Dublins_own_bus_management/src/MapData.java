...
public class MapData
{
    ...
    private ArrayList<Edges>[] adj;
    private int V;
    private AlgorithmsResults sps;
    
    //Constructor for a BusStopMap object, with parameters being paths to all three input files.
    protected MapData(String stopsFile, String stopTimesFile, String transfersFile) throws IOException
    {
        ...
        long numberOfLinesToRead = ((UtilityWork.numberOfLinesInAFile(stopsFile)-1) + (UtilityWork.numberOfLinesInAFile(stopTimesFile)-1) + (UtilityWork.numberOfLinesInAFile(transfersFile)-1));
        ...
        while((line = reader.readLine()) != null)
        {
            ...
            {
                    UtilityWork.updateProgressBar(counterForProgressBar, numberOfLinesToRead);
            }
        }
        reader.close();
        V = vertex;
        ...
        adj = new ArrayList[V];
        for(int i = 0; i < V; i++) adj[i] = new ArrayList<Edges>();
        ...
        while((line = reader.readLine()) != null)
        {
            ...
            adj[fromIndex].add(new Edges(fromIndex, toIndex, weight));
            counterForProgressBar++;
            //We only want to update the progress bar for every 1% of progress, so the function remains speedy.
            //If we updated the progress bar after every line, the I/0 required would slow down the function.
            if (counterForProgressBar % ((int)numberOfLinesToRead/100) == 0)
        ...
        while((line = reader.readLine()) != null)
        {
            ...
            if(currentTripID == lastTripID)
            {
                fromIndex = stopIndexes.get(fromID);
                toIndex = stopIndexes.get(toID);
                adj[fromIndex].add(new Edges(fromIndex,toIndex, 1.0));
            }
        ...
        UtilityWork.updateProgressBar((int)numberOfLinesToRead, numberOfLinesToRead);
        reader.close();
    }
    ...
    protected void makePaths(String sourceStop)
    {
        if(stops.containsKey(sourceStop))
        {
            ...
            sps = new AlgorithmsResults(this, stopIndexes.get(stopID));
        }
        else throw new IllegalArgumentException();
    }
     ...
     */
    protected ArrayList<Edges> adjacent(int vertex)
    { return adj[vertex]; }
}
